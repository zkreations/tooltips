...
CSS 変数を使用せず、できるだけ軽量にしたい場合のために、本プロジェクトにはコンパクト版（[compact.css](compact.css) / `compact.min.css`）が含まれています。CSS Anchor Positioning による自動配置と、非対応ブラウザ向けのフォールバックをそのまま維持しています。
