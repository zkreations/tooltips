...
非常に軽量（非圧縮で約 1 KB、圧縮後で約 800 B）であるため、外部依存関係として読み込むのではなく、`compact.css` のコードをプロジェクトのスタイルシートに直接コピーして組み込むことを推奨します。これにより、変数に頼ることなく CSS で直接ツールチップのスタイル全体をカスタマイズできます。
